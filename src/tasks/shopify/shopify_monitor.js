...
export async function scanXML(config: ShopifyConfig) {
  const url = `${config.base_url}/sitemap_products_1.xml`;
...
export async function scanAtom(config: ShopifyConfig) {
  ...
  try {
    ...
    try {
      ...
      // scan all items
      const scanResults = entries.filter(
        entry => containsKeywords(entry.title[0], config.keywords)
      );
